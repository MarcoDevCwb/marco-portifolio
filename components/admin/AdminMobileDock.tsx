...
export function AdminMobileDock({
  ...
  const slides = useMemo(() => chunkItems(dockItems, ITEMS_PER_SLIDE), [])

  const activeSlideFromRoute = useMemo(() => {
    const foundIndex = slides.findIndex((slide) =>
      slide.some((item) => item.href === activeHref)
    )

    return foundIndex >= 0 ? foundIndex : 0
  }, [activeHref, slides])

  const [activeSlide, setActiveSlide] = useState(activeSlideFromRoute)

  useEffect(() => {
    setActiveSlide(activeSlideFromRoute)
  }, [activeSlideFromRoute])
        ...
        >
          ...
          <div className="relative flex items-center justify-between gap-2">
            <button
              ...
              disabled={!canGoPrev}
              className={[
                "inline-flex h-7 w-7 shrink-0 items-center justify-center transition-all duration-300 ease-out",
                canGoPrev
